...
from nvd3 import (cumulativeLineChart, discreteBarChart,
                  lineChart, linePlusBarChart,
                  lineWithFocusChart, multiBarChart,
                  multiBarHorizontalChart, pieChart,
...
class NVD3DirectiveBase(rst.Directive):
    ...
    def run(self):
        ...
        for index in range(length):
            ...
            chart.add_serie(y=self.dataset['ydata'][index], x=self.dataset['xdata'],
                            name=self.dataset['data_keys'][index],
                            extra=self.extra_serie, **kwargs)
...
def setup(app):
    ...
    rst.directives.register_directive('nvd3-cumulativelinechart',
                                      CumulativeLineChartDirective)
    rst.directives.register_directive('nvd3-discretebarchart',
                                      DiscreteBarChartDirective)
    rst.directives.register_directive('nvd3-linechart',
                                      LineChartDirective)
    rst.directives.register_directive('nvd3-lineplusbarchart',
                                      LinePlusBarChartDirective)
    rst.directives.register_directive('nvd3-linewithfocuschart',
                                      LineWithFocusChartDirective)
    rst.directives.register_directive('nvd3-multibarchart',
                                      MultiBarChartDirective)
    rst.directives.register_directive('nvd3-multibarhorizontalchart',
                                      MultiBarHorizontalChartDirective)
    rst.directives.register_directive('nvd3-piechart',
                                      PieChartDirective)
    rst.directives.register_directive('nvd3-scatterchart',
                                      ScatterChartDirective)
    rst.directives.register_directive('nvd3-stackedareachart',
